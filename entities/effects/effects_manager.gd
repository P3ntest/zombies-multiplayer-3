...
func spawn_blood_splat(start_position: Vector2, direction: Vector2):
    assert(multiplayer.is_server(), "Only the server can spawn blood splats.")

    _spawn_blood_splat_net.rpc(start_position, direction)

@rpc("authority", "call_local", "unreliable")
func _spawn_blood_splat_net(start_position: Vector2, direction: Vector2):
